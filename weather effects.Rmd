...
maxfatalities
```
In terms of fatalities, the worst event type was `r maxfatalities$EVTYPE.cor` in `r maxfatalities$year` with a total of `r maxfatalities$deaths` deaths. In terms of injuries, the worst event type was `r maxinjuries$EVTYPE.cor` in `r maxinjuries$year` with a total of `r maxinjuries$injuries` injuries.

## Injuries and Fatalities over Time

Has the number of injuries or fatalities changed over time? On the one hand given larger populations it would not be unreasonable to expect injuries and fatalities to increase. However, advances in technology (building codes, etc) along with better forecasting suggest that injuries and fatalities should decrease.


```{r, echo=TRUE}
plot1<-ggplot(evtype.totals,aes(x=year,y=injuries))+geom_line()
plot1
```
```{r, echo=TRUE}
plot2<-ggplot(evtype.totals,aes(x=year,y=deaths))+geom_line()
plot2
```
